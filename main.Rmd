...
subtitle: "Une analyse par les méthodes de régression sur variables latentes sur les ventes de propriétés résidentielles entre 2006 et 2010 à Ames, une ville de l’Etat de l’Iowa aux Etats-Unis."
output: 
  bookdown::gitbook:
   #word_document:
...
github-repo: "agailloty/latent-variable-regressions"
#toc: true
---

```{r message=FALSE, warning=FALSE, include=FALSE}
pkg <- c("tidyverse", "plsr", "caret", "pls", "recipes", "dplyr", "psych", "tidytext",
         "AmesHousing", "ggcorrplot", "factoextra", "hrbrthemes", "bookdown", "pander", "vip")
...
knitr::opts_chunk$set(dpi = 350, message = FALSE, 
                      ...
                      fig.height = 8, fig.width = 8, dpi=300, comment = NA)
source("functions.R")
kable <- knitr::kable
...
Le graphique suivant présente les histogrammes ou diagramme e bâton de toutes les variables quantitatives présentes dans le jeu de données . Ce type de représentation a pour intérêt d'afficher les caractéristiques des variables qui n'étaient pas observables par les simples statistiques que nous avions présentées plus haut.  
...
data.frame(pc$var$contrib[, 1:2]) %>%
  ...
  slice(1:10) %>%
  kable(caption = "Les dix premières contributions à la deuxième composante")
```
...
Le modèle se présentera sous forme d'une équation comme celle-ci:
$$ SalePrice = \alpha + \beta * X $$
où $\alpha$ est la constante et $X$ les variables explicatives et $\beta$ les coefficients associés à chacune des variables explicatives.  
...
- La régression sur les moindres carrés partiels, qui équilibre les deux objectifs d’explication de la variation de la variable endogène et expliquant la variation des prédicteurs. Deux formulations différentes pour les moindres carrés partiels sont disponibles : 
...
# Le principe de la méthode de la régression sur les composantes principales
...
## Régression sur composantes principales (RCP ou PCR)
...
for (i in 2:5) df <- rbind(df, retrieve_dims_contrib(pc, i))
```
...
```{r fig.height=7, fig.width=12}
df %>% mutate(variable = tidytext::reorder_within(variable, contrib, dim)) %>%
...
## Limite de la régression sur les composantes principales
...
de ce fait à de mauvaises interprétations des résultats.

Nous pouvons donc utiliser la deuxième méthode de cette famille.

\pagebreak

# Régression sur les moindres carrés partiels
...
Avec : R matrice des résidus associés à la prédiction de $X$ ;
$F$ vecteur des résidus associés à la prédiction de $y$.  
...
Avec $t_1$ de dimension (n,1)
$p_1$ de dim (1,m) loadings
$q_1$ de dim (1,1) loadings
...
## Modèle PLSR avec validation croisée
...
ncomp.onesigma <- selectNcomp(model.plsr, method = "onesigma", plot = TRUE, alpha = 0.5)
```
...
data.frame(model.plsr$coefficients) %>%
  tibble::rownames_to_column("variables") %>%
  reshape2::melt() %>%
  mutate(variable = as.character(variable),
    comp = paste("comp", substr(variable, 12, 12)),
    variable = tidytext::reorder_within(variable, value, comp)) %>%
  select( - variable) %>%
  filter(comp == "5 composantes") %>%
...
plot(model.plsr, plottype = "coef", ncomp=1:5, legendpos = "bottomleft", xlab = "nm")
```
...
## Loadings (chargements) and weights (poids)

Les « Loadings » reflètent (mais ne sont pas) les corrélations entre les composantes et les variables explicatives. Ils correspondent aux coordonnées sur les axes factoriels. On considère qu’une valeur supérieure à 0,4 (en valeur absolue) indique une liaison significative. Ce seuil est arbitraire (on
descend parfois à 0,25). Les variables les plus excentrées sont les plus représentatives (ont une CTR plus élevée).

Le poids des variables expliquées reflète les corrélations entre ces variables et les scores uh. Ils permettent de cerner ce qui est expliqué sur ces facteurs.

Le poids des variables explicatives reflète la corrélation de ces variables avec les scores uh. Ils indiquent le rôle de chaque explicative dans l’explication globale de chaque axe.
Dans la pratique, poids et loadings positionnent les variables de manière similaire, les interprétations sont identiques.



```{r}
Yloadings(model.plsr)
```
...
## Performances prédictives du modèle de régression multiple
...
sigma(mod)
```
Le RMSE pour ce modèle est de 35861, ce qui signifie qu'en moyenne les valeurs prédites dévient de la valeur observée de 35861 dollars en valeur absolue.  

## Performances prédictives du modèle de régression sur les composantes principales
...
sigma(mod_pc)
```
Le RMSE de ce modèle de régression sur les composantes principales est de 39223, soit 3361 de plus que le modèle de régression multiple.


## Performances prédictives du modèle sur les moindres carrés partiels
...
Les valeurs affichées sont les RMSE pour chaque composante supplémentaire retenue dans le modèle. Pour les 5 composantes que nous avons retenues dans le modèle des moindres carrés partiels, le RMSE est de plus de 42000. 
...
Daniel Christophe, Régressions sur variables latentes : Principal Component Regression et Partial Least Square (Cours de Data Mining 2)
...
## Annexe 2:  Les coefficients estimés par le PLS
...
coef(model.plsr) %>%
  data.frame() %>%
  setNames(., c("Sale_Price")) %>%
kable()
```

## Annexe 3 : Description des colonnes :
