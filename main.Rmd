...
pkg <- c("tidyverse", "plsr", "caret", "pls", "recipes", "dplyr", "psych", "tidytext",
         "AmesHousing", "ggcorrplot", "factoextra", "hrbrthemes", "bookdown", "pander")
...
Le cercle des corrélations montre qu'une grande partie des variable est fortement corrélée avec l'axe 1. Ce qui signifie que ces variables évoluent dans le même sens.
...
# La régression régression multiple
...
Dans le cas d'un bien immobilier, la taille des pièces du bien dépend de la superficie totale de la maison.  Cette situation nous amène à des résultats potentiellement biasés et non généralisables pour expliquer le phénomène.   
...
- La régression sur les moindres carrés partiels, qui équilibre les deux objectifs d’explication de la variation de l'a réponse'endogène et expliquant la variation des prédicteurs. Deux formulations différentes pour les moindres carrés partiels sont disponibles: 
...
Le poids des variables sur les trois premières dimensions est assez équilibrée avec des amplitudes modérées (0 à 12). Les deux dernières dimensions sont chacune fortement dominées par une variable avec des amplitudes beaucoup plus grande (0 à 30).  
...
Avec : R matrice des résidus associées à la prédiction de $X$ ;
...
## Les coefficients estimés par le PLS
...
Le poids des variables explicatives reflètent la corrélation de ces variables avec les scores uh. Ils indiquent le rôle de chaque explicative dans l’explication globale de chaque axe.
Dans la pratique, poids et loadings positionnent les variables de manière similaire, les interprétations sont identiques.

```{r}
model.plsr$loading.weights
```
