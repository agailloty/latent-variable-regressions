...
```{r}
kable(retrieve_dims_contrib(pc, dim = 1),
      caption = "Les contributions des variables aux différents axes factoriels")
```

```{r}
kable(retrieve_dims_contrib(pc, dim = 2),
      caption = "Les contributions des variables aux différents axes factoriels")
```


```{r}
kable(retrieve_dims_contrib(pc, dim = 3),
      caption = "Les contributions des variables aux différents axes factoriels")
```


```{r}
kable(retrieve_dims_contrib(pc, dim = 4),
      caption = "Les contributions des variables aux différents axes factoriels")
```


```{r}
tidy_all_dims <- function(pc, ndims = 5, keep = 10){
  df <- data.frame(retrieve_dims_contrib(pc, 1, keep = keep), dim = "Dim.1")
  
  for (i in 2:ndims){
    df_i <- data.frame(retrieve_dims_contrib(pc, i, keep = keep), dim = paste0("Dim.", i)
    df <- rbind(df, df_i)
  }
  return(df)
}
